...
class ArticleInfo:
    ...
    # v2.6.0: Model organism detection
    model_organism: str = "human"  # "human", "mouse", "rat", "non-human primate", etc.
...
class KeyFinding:
    ...
    biomarkers: List[str] = field(default_factory=list)  # Mentioned biomarkers (v2.6.0)
...
class KeyFindingsExtractor:
    ...
    # Biomarker patterns (v2.6.0)
    BIOMARKER_PATTERNS = [
        r"BDNF", r"cathepsin\s*B", r"CTSB", r"cortisol", r"CRP", r"IL-6", r"interleukin",
        r"myokine", r"polymorphism", r"Val66Met", r"amyloid", r"tau", r"insulin",
        r"Hba1c", r"testosterone", r"estrogen", r"ghrelin", r"leptin", r"orexin", r"hypocretin"
    ]
    ...
    def _compile_patterns(self):
        ...
        self.biomarker_regex = [re.compile(rf"\b{p}\b", re.IGNORECASE) for p in self.BIOMARKER_PATTERNS]

    def _extract_biomarkers(self, text: str) -> List[str]:
        """Extract mentioned biomarkers from text."""
        found = []
        for regex in self.biomarker_regex:
            match = regex.search(text)
            if match:
    ...
    def extract(self, article: ArticleInfo) -> KeyFinding:
        ...
        biomarkers = self._extract_biomarkers(article.abstract)
...
class PubMedClient:
    ...
    async def search(self, query: str, max_results: int = 10, human_only: bool = False) -> List[str]:
        """Search PubMed and return list of PMIDs"""
        term = query
        if human_only:
            term = f"({query}) AND humans[Filter]"
    ...
    def _parse_article_xml(self, xml_text: str, pmid: str) -> Optional[ArticleInfo]:
        ...
        try:
            ...
            animal_patterns = {
                "mouse": [r"mice", r"mouse", r"murine"],
                "rat": [r"rat", r"rattus"],
                "non-human primate": [r"macaque", r"monkey", r"primate"],
                "zebrafish": [r"zebrafish"],
                "drosophila": [r"drosophila", r"fruit fly"]
            }
            
            combined_text = (title + " " + abstract).lower()
            
            for mesh in article.findall(".//MeshHeading/DescriptorName"):
                term = mesh.text if mesh.text else ""
                if term:
                    mesh_terms.append(term)
                    # Use MeSH for reliable detection
                    term_lower = term.lower()
                    if "mice" in term_lower or "mouse" in term_lower:
                        model_organism = "mouse"
                    elif "rats" in term_lower or "rat" in term_lower:
                        model_organism = "rat"
                    elif "primates" in term_lower and "humans" not in term_lower:
                        model_organism = "non-human primate"
            
            # Fallback to text analysis if MeSH is inconclusive
            if model_organism == "human":
                for org, patterns in animal_patterns.items():
                    if any(re.search(rf"\b{p}\b", combined_text) for p in patterns):
                        model_organism = org
                        break
...
class ResearchSynthesizer:
    ...
    EQUITY_PATTERNS = [
        r"low-income", r"socioeconomic", r"disparity", r"inequality", r"underserved",
        r"marginalized", r"racial", r"ethnic", r"minority", r"rural", r"urban",
        r"accessibility", r"affordability", r"educational attainment", r"uninsured"
    ]
    ...
    def __init__(self, client: PubMedClient, analyzer: TrustAnalyzer):
        ...
        self.equity_regex = [re.compile(rf"\b{p}\b", re.IGNORECASE) for p in self.EQUITY_PATTERNS]

    def _extract_equity_data(self, articles: List[ArticleInfo]) -> List[str]:
        """Extract health equity considerations from a batch of articles."""
        equity_notes = []
        for a in articles:
            for regex in self.equity_regex:
                match = regex.search(a.abstract)
                if match:
                    equity_notes.append(f"Mentioned in PMID {a.pmid}: {match.group(0)}")
        
        if not equity_notes:
            return ["No specific health equity data detected in abstracts. Most studies may focus on standard clinical populations."]
        return list(set(equity_notes))[:5]
    
    async def synthesize(self, query: str, max_articles: int = 7) -> Dict[str, Any]:
        # v2.6.0: Detect clinical complexity to prioritize human studies
        from datetime import datetime
        
        # Check if query is likely clinical/human focused
        human_indicators = ["patient", "treatment", "therapy", "clinical", "human", "adult", "child", "elderly"]
        is_clinical = any(ind in query.lower() for ind in human_indicators)
        
        pmids = await self.client.search(query, max_articles, human_only=is_clinical)
        ...
        return {
            ...
            "top_articles": [
                {
                    ...
                    "model_organism": a.model_organism,
            ...
            "health_equity": self._extract_equity_data(articles),
...
class MCPServer:
            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "max_articles": {
                            "type": "integer",
                            "description": "Articles to analyze (default: 7, max: 15)",
                            "default": 7
                        }
                    },
                    "required": ["query"]
            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "max_results": {
                            "type": "integer",
                            "description": "Max articles to export when using query (default: 7, max: 50)",
                            "default": 7
                        }
                    },
                    "required": ["format"]
    ...
    async def _handle_research_summary(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        query = args.get("query", "")
        max_articles = min(args.get("max_articles", 7), 15)
    ...
    async def _handle_export_citations(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_results = min(args.get("max_results", 7), 50)
